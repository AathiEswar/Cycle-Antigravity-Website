  ...
  return (
    <section ref={containerRef} className="relative h-screen w-full flex flex-col justify-center items-center overflow-hidden bg-track-dark">
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-track-dark/20" />
      </div>
      ...
      <div className="z-10 text-center text-white">
        <div className="overflow-hidden" ref={textRef}>
          <h1 className="font-display text-[15vw] leading-[0.8] font-bold tracking-tighter flex justify-center">
            {siteContent.hero.title.part1.split("").map((char, i) => <span key={i} className="hero-char inline-block">{char}</span>)}
            <span className="w-[4vw]"></span>
            {siteContent.hero.title.part2.split("").map((char, i) => <span key={i + 10} className="hero-char inline-block text-track-green">{char}</span>)}
          </h1>
        </div>
      ...
      <div className="max-w-[1800px] mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
        <div className="md:col-span-4 z-10">
          <div className="detail-text-item mb-8">
            <span className="text-track-green font-tech text-xs uppercase tracking-[0.2em]">{siteContent.craftedDetail.badge}</span>
          ...
          <div className="detail-text-item max-w-sm">
            ...
            <div className="mt-8 flex gap-8">
              {siteContent.craftedDetail.stats.map((stat, i) => (
                <div key={i}>
                  <h4 className="text-3xl font-display text-white">{stat.value.replace(/[a-z%]/g, "")}<span className="text-track-green text-lg">{stat.value.match(/[a-z%]/g)}</span></h4>
                  <p className="text-track-gray text-xs uppercase tracking-wider mt-1">{stat.label}</p>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <section className="py-40 px-6 bg-track-dark flex justify-center items-center">
      <h2 ref={text} className="text-4xl md:text-7xl lg:text-8xl font-display text-white font-bold text-center leading-[0.9] tracking-tighter max-w-6xl mx-auto opacity-100">
        {siteContent.statement.lines.map((line, i) => (
          <span key={i} className={`block ${i === 1 ? 'text-track-gray' : i === 2 ? 'text-track-green' : ''}`}>
            {line}
          </span>
        ))}
      </h2>
  ...
  return (
    <section ref={triggerRef} className="h-screen bg-track-charcoal overflow-hidden relative">
      <div className="absolute top-12 left-6 md:left-12 z-10">
        ...
        <p className="text-track-gray font-tech text-xs md:text-sm mt-2">01 / 0{siteContent.bikes.length}</p>
      </div>
    ...
    <section ref={container} className="py-32 bg-track-dark relative overflow-hidden min-h-[50vh]">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="mb-20 text-center">
          <p className="text-track-green font-tech text-xs uppercase tracking-[0.3em] mb-4">{siteContent.specs.badge}</p>
          <h2 className="text-3xl md:text-5xl font-display font-bold uppercase tracking-tighter text-white">
            {siteContent.specs.title.split(" ")[0]} <span className="text-track-green">{siteContent.specs.title.split(" ")[1]}</span>
          </h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {siteContent.specs.features.map((f, i) => (
            <div key={i} className="spec-card group relative p-10 bg-white/5 border border-white/10 hover:border-track-green/50 hover:bg-white/10 transition-all duration-500 overflow-hidden opacity-0">
    ...
    <section ref={container} className="py-32 bg-track-charcoal text-white overflow-hidden">
      <div className="max-w-[1800px] mx-auto px-6 md:px-12">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20">
          <div>
            <h2 className="text-5xl md:text-7xl font-display font-bold uppercase tracking-tighter">{siteContent.community.title}</h2>
            <p className="font-tech text-track-gray mt-4 max-w-md">{siteContent.community.description}</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {siteContent.community.items.map((img, i) => (
    ...
    <section ref={container} className="py-24 bg-track-dark border-t border-white/5">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-20 items-center">
          <div>
            <h2 className="contact-item text-track-green font-tech text-xs uppercase tracking-[0.3em] mb-6 font-bold">{siteContent.contact.badge}</h2>
            <h3 className="contact-item text-4xl md:text-6xl font-display font-bold text-white tracking-tighter leading-none mb-12">
              {siteContent.contact.title[0]} <br /> {siteContent.contact.title[1]}
            </h3>
            <div className="contact-item flex items-center gap-6 group cursor-pointer">
              ...
              <div>
                <p className="text-white/40 font-tech text-[10px] uppercase tracking-widest mb-1">{siteContent.contact.phoneLabel}</p>
                <p className="text-white text-2xl font-display font-medium">{siteContent.contact.phone}</p>
              </div>
            </div>
          </div>
          <div className="bg-track-charcoal p-12 relative overflow-hidden group">
            ...
            <h4 className="contact-item text-white font-display text-2xl font-bold mb-4">{siteContent.contact.locationTitle}</h4>
            ...
            <div className="contact-item mt-12">
              <Magnetic>
                <div className="inline-flex items-center gap-2 text-track-green font-tech text-xs uppercase tracking-widest cursor-pointer border-b border-track-green pb-1 hover:gap-4 transition-all">
                  {siteContent.contact.mapsButton} <ChevronRight className="w-4 h-4" />
                </div>
              </Magnetic>
            </div>
          </div>
  ...
  <footer className="bg-black text-white py-24 px-6 border-t border-white/10 relative z-10">
    <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start gap-12">
      <div className="max-w-xs">
        ...
        <p className="font-tech text-white/40 text-[10px] uppercase tracking-widest mb-4">{siteContent.footer.locationLabel}</p>
